...
// Q_INIT_RESOURCE must be called from global scope (it declares an extern
// function; inside a namespace the symbol would be mangled into it), and it
// must be called explicitly: a plugin's compiled-in resources are not yet
// registered while its constructor runs, so tr() would silently return the
// untranslated source strings.
static void init_date_resources() { Q_INIT_RESOURCE(date_lang); }
