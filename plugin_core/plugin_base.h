...
public:
  ...
  /*!
   * @brief Set widget to use as a parent for plugin's dialogs.
   *
   * A dialog created without a parent is only "transient for group" for the
   * window manager, so there is no reason for it to be kept above the settings
   * window. As plugin dialogs are application modal, such a dialog blocks the
   * settings window while it may be stacked behind it - the settings window
   * then looks like it hangs. Plugin management system calls this before
   * Configure() to give plugins a proper parent window.
   * @param parent - widget to use as a dialog parent
   * @see DialogParent()
   */
...
protected:
  /*!
   * Widget to pass as a parent to any dialog created by plugin.
   * @return parent widget, may be nullptr
   * @see SetDialogParent()
   */
