...
> **后续更新（2026-08-05）**：这里说的"实验性选项"页已整体撤销，`OPT_ALLOW_OVER_PANELS`
> 这个设置项本身也已彻底删除——覆盖面板改成了 Linux 下无条件默认行为，不再有开关可关。
> 唯一仍然覆盖不了的是 GNOME Shell 自己的顶栏（属于该桌面环境的限制，非本程序可修）。
> 详见 `MODIFICATIONS.md` 的"'Cover Panels' removed as a setting"与"diagnosed: GNOME
> Shell limitation"两节。本条历史记录原样保留，不代表当前设置界面仍有这个选项。
