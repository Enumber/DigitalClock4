...
* **想覆盖任务栏/面板/Dock 也可以** —— 托盘图标菜单和时钟自己的右键菜单（紧跟
  在"位置"下面）都有一个"可覆盖面板"勾选项，跟设置里"杂项"标签下的同名开关
  实时互相同步，在任意一处切换都立即生效并生效即存，不用打开设置对话框。
* **大部分设置改了立刻生效** —— 全屏检测、多工作空间显示、保持可见、点击移动、
  任务栏图标显示、Linux 优化的"始终置顶"模式、在所有显示屏上显示时钟，这些都
  不用重启程序。同一时间只会有一个时钟实例在跑，再启动一次只会把已经在跑的那个
  拉到前台。
