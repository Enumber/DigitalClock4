...
* **可靠的多屏位置恢复** —— 新安装在有多块屏幕时默认优先非主屏；每块物理显示器
  分别保存位置，屏幕断线时自动迁移到可用屏幕，重连或重启后回到原屏原位置。
* **默认就能覆盖任务栏/面板/Dock、始终够得着、不出现在 Alt-Tab 里——不用配置**：
  Linux 下"始终置顶"永远用更强硬的、绕过窗口管理器的方式，所以时钟本来就压在
  面板/Dock 之上（唯一的例外是 GNOME Shell 自己的顶栏，任何普通应用都盖不住它，
  原因见 [MODIFICATIONS.zh-CN.md](MODIFICATIONS.zh-CN.md)）；窗口永远不会出现
  在任务栏或 Alt-Tab 切换器里；也永远不会被拖到完全看不见的地方。这些以前都是
  勾选框，现在就是时钟本来的样子。
* **剩下的大部分设置改了立刻生效** —— 全屏检测、多工作空间显示、点击移动、
  在所有显示屏上显示时钟，这些都不用重启程序。同一时间只会有一个时钟实例在跑，
  再启动一次会直接被挡下退出，已经在跑的那个不受影响。
...
* 设置里自带「开机启动」开关，装完在设置里打开即可，不用手动配置
  （安装器本身不会替你打开自启动）
* 没有更新检查、没有更新提示；主程序本身不会主动联网——4.7.9 已是 4.x 最后一版，
  也确实没什么可查的了（唯一的例外是可选的 IP 地址插件，不手动开启就不会联网）
...
要不要桌面图标。桌面图标已标记为可信，双击直接启动，不会弹「是否允许启动」
确认框。开机自启动不在安装询问里：装完在程序的「设置 → 杂项」里打开即可
（脚本化场景可用 `--autostart`）。
