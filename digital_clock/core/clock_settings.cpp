...
QMap<Option, QVariant> ClockSettings::GetSettings() const
{
  ...
  all_settings[OPT_TRANSPARENT_ON_HOVER]  = GetValue(OPT_TRANSPARENT_ON_HOVER);
...
QString ClockSettings::GetKey(const int id) const
{
  switch (static_cast<Option>(id)) {
    ...
    case OPT_TRANSPARENT_ON_HOVER:  return "window/transparent_on_hover";
...
QVariant ClockSettings::GetDefaultValue(const int id) const
{
  ...
  switch (static_cast<Option>(id)) {
    ...
    case OPT_TIME_FORMAT:           return QString("HH:mm");
    case OPT_ALIGNMENT:             return static_cast<int>(CAlignment::A_RIGHT);
