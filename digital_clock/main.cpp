...
int main(int argc, char* argv[])
{
  ...
  QApplication::setApplicationVersion("4.7.9");
...
#endif
  // the real application icon (as on Windows), not the small menu glyph:
  // this is what the taskbar, Alt+Tab and the window manager show
  app.setWindowIcon(QIcon(":/clock/images/app-icon.png"));
  ...
  for (QString locale : QLocale::system().uiLanguages()) {
    ...
    if (app_translator.load(QLatin1String(":/digital_clock/lang/digital_clock_") + locale)) {
      ...
      // Qt's own strings (dialog buttons, the font and colour pickers...).
      // Look next to the executable first so a portable install is complete on
      // its own — the Windows build ships them in translations/ the same way —
      // and fall back to the system Qt translations when running from a build
      // tree or a distro package.
      const QString qt_tr = QLatin1String("qt_") + locale;
      if (qt_translator.load(qt_tr, app.applicationDirPath() + QLatin1String("/translations")) ||
          qt_translator.load(qt_tr, QLibraryInfo::location(QLibraryInfo::TranslationsPath)))
