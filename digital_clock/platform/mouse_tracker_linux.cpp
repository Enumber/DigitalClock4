...
private:
  // written by stop() from the GUI thread, read by run() in the tracker thread
...
void MouseTrackerPrivate::run()
{
  ...
  const int dpy_fd = ConnectionNumber(dpy);
  ...
  while (!should_stop_) {
    ...
    // Never call XNextEvent() unless an event is already there: it blocks
    // forever when the pointer does not move, and stop() would then wait
    // forever for this thread to notice should_stop_. That is exactly what
    // used to happen on "Quit" - the whole application hung in its destructor
    // until the user happened to move the mouse. Poll the connection with a
    // timeout instead so the flag is checked regularly.
    if (!XPending(dpy)) {
      struct pollfd pfd;
      pfd.fd = dpy_fd;
      pfd.events = POLLIN;
      pfd.revents = 0;
      if (poll(&pfd, 1, 200) <= 0)
        continue;                   // timeout, error or interrupted: re-check should_stop_
      if (!XPending(dpy))
        continue;
    }
...
void MouseTracker::stop()
{
  ...
  impl_->stop();
  // wait() properly blocks on the thread instead of burning a core spinning on
  // isFinished(); the timeout keeps a wedged X connection from hanging exit.
  if (!impl_->wait(2000)) {
    impl_->terminate();
    impl_->wait(1000);
  }
